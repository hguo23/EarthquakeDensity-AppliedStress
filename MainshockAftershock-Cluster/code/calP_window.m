...
if ~isempty(p)
    for i = 1:sl:length(p)
        ...
        x = log10(p(I));
        I = isoutlier(x,'percentiles',[0 100]);
        x = x(~I);
        
        n = min([50 round(length(x)/3)]); % bin numbers for histogram
        [count,binedge] = hist(x,n);
        I = count>0; count = count(I); binedge = binedge(I);
        if sum(I)>10 % enough bins for fitting
